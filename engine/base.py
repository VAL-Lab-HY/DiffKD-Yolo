...
class BaseTrainer:
    """Base trainer với Knowledge Distillation (KD/DiffKD) support.

    Thứ tự khởi tạo KD:
        _setup_train()
            ├── setup_model()          — load student
            ├── _setup_kd()            — load teacher + khởi tạo KDLoss   ← TRƯỚC optimizer
            └── _build_train_pipeline()
                    └── build_optimizer()  — include DiffKD params         ← SAU _setup_kd
    """
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        self.teacher_name   = overrides.pop("teacher_name", None)
        self.student_name   = overrides.pop("student_name", None)
        self.kd_loss_weight = overrides.pop("kd_loss_weight", 1.0)
        self.kd_method      = overrides.pop("kd_method", "diffkd")
        ...
        if isinstance(_teacher_raw, (str, Path)):
            ...
            if _teacher_raw is not None:
                LOGGER.warning(
                    ...
                    "Truyền nn.Module trực tiếp không hỗ trợ (DDP). Distillation bị tắt."
                )
        ...
        if self.ddp:
            ...
            if self.args.batch < 1.0:
                raise ValueError(
                    ...
                    f"please specify a valid batch size multiple of GPU count {self.world_size}."
                )
            if self.teacher_path is not None:
                ...
                self.args.kd_method      = self.kd_method
    ...
    def _setup_kd(self):
        """Load teacher + khởi tạo KDLoss. Gọi trước build_optimizer."""
        if self.teacher_path is None or self.kd_loss_fn is not None:
            return
        ...
        if isinstance(ckpt, dict):
            ...
            if raw is None:
                raise ValueError(
                    f"KD: checkpoint '{self.teacher_path}' không có key "
                    f"'state_dict'/'model'/'ema'. Keys hiện có: {list(ckpt.keys())}"
                )
        else:
        ...
        if isinstance(raw, dict):
            ...
            self.teacher = raw  # nn.Module trực tiếp
        ...
        LOGGER.info(
            ...
            f"method={self.kd_method} | weight={self.kd_loss_weight}"
        )
    ...
    def _setup_train(self):
        ...
        # ✅ Optimizer SAU _setup_kd → build_optimizer có thể thêm DiffKD params
                ...
                try:
                    with autocast(self.amp):
                        ...
                        if self.kd_loss_fn is not None:
                            # KD mode: lấy raw preds, KDLoss xử lý toàn bộ
                            preds = unwrap_model(self.model)(batch["img"])
    ...
    def auto_batch(self, max_num_obj=0):
        return check_train_batch_size(
            model=self.model, imgsz=self.args.imgsz, amp=self.amp,
            batch=self.batch_size, max_num_obj=max_num_obj,
        )
    ...
    def build_teacher_model(self) -> torch.nn.Module:
        """Override để khởi tạo custom teacher architecture khi checkpoint là state_dict.

        YOLO .pt checkpoints lưu nn.Module trực tiếp → không cần override.
        Chỉ cần override khi teacher là custom model (vd IRFormer) lưu dạng state_dict.
        ...
        raise NotImplementedError(
            "KD: teacher checkpoint chứa state_dict nhưng build_teacher_model() chưa được override. "
            "Override method này trong subclass để trả về teacher model instance."
        )
    ...
    def check_resume(self, overrides):
        ...
        if resume:
            try:
                ...
                for k in (
                    "imgsz", "batch", "device", "close_mosaic", "augmentations",
                ...
                if ckpt_args.get("augmentations") is not None:
                    LOGGER.warning(
                        ...
                        "being restored. Pass 'augmentations' again to preserve them."
                    )
            except Exception as e:
                raise FileNotFoundError(
                    "Resume checkpoint not found. Pass a valid checkpoint, "
    ...
    def resume_training(self, ckpt):
        ...
        if self.epochs < start_epoch:
            LOGGER.info(f"{self.model} trained for {ckpt['epoch']} epochs. Fine-tuning for {self.epochs} more.")
    ...
    def build_optimizer(self, model, name="auto", lr=0.001, momentum=0.9, decay=1e-5, iterations=1e5):
        ...
        # ✅ Thêm DiffKD params — kd_loss_fn đã được init trong _setup_kd() trước đó
        if self.kd_loss_fn is not None and hasattr(self.kd_loss_fn, "diff"):
            kd_params = [p for p in self.kd_loss_fn.diff.parameters() if p.requires_grad]
            if kd_params:
                optimizer.add_param_group({
                    "params":       kd_params,
                    "lr":           lr,
                    "weight_decay": 0.0,  # không decay cho adapter/AE
                    "param_group":  "diffkd",
                })
                LOGGER.info(
                    f"{colorstr('KD:')} DiffKD trainable params="
                    f"{sum(p.numel() for p in kd_params):,} added to optimizer"
                )
